  ...
  <div class="shops-list main-content">
    ...
    <div class="content">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <!-- <img :src="scope.row.agent_name" alt="" width="60" height="60"> -->
          </template>
        </el-table-column>
        <el-table-column
          prop="agent_account"
      ...
      /> -->
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.shops.length}}</span>
          </template>
        </el-table-column> 
        <el-table-column
        :label="$t('备注')"
    ...
    <el-dialog v-dialogDrag center :title="dialogType=='add'? '新增代理商': dialogType=='edit'? `${$t('编辑代理商')}`: `${$t('代理商详情')}`" :visible.sync="dialogFormVisible">
      <el-form ref="shopForm" :model="shopForm" :rules="rules" label-width="100px" label-position="left" size="small">
        ...
        <el-form-item :label="$t('授权数量')" prop="total_number">
          <el-input v-model="shopForm.total_number " :disabled="dialogType=='detail'" />
        </el-form-item>
        <el-form-item v-if="dialogType!=='detail'" :label="$t('密码')" prop="agent_pwd">
          <el-input v-model="shopForm.agent_pwd " :disabled="dialogType=='detail'" />
        </el-form-item>
        <el-form-item v-if="dialogType!=='detail'" :label="$t('备注')" prop="remark">
          <el-input v-model="shopForm.remark" :disabled="dialogType=='detail'" />
        </el-form-item>
...
<script>
import { getList, delShop, addShop, editShop } from '@/api/agent'
...
export default {
  ...
  methods: {
    ...
    showDialog(type, form) {
      ...
      if (form && form.agent_id) {
        // 请求分类详情
        this.shopForm.agent_id = form.agent_id
        this.shopForm.agent_name = form.agent_name
        this.shopForm.agent_account = form.agent_account
        this.shopForm.total_number = form.total_number
        this.shopForm.agent_pwd = form.agent_pwd
        this.shopForm.expire_time = form.expire_time
        this.shopForm.remark = form.remark
